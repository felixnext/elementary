...
def similarVectorDoc(text):
  ...
  #vector = doc2vec_model.infer_vector([word.strip(string.punctuation) for word in text.split()])
  return vector
...
def mostSimilarText(text):
  global doc2vec_model
  vectors = doc2vec_model.most_similar(positive=nltk.word_tokenize(text))
  return vectors
